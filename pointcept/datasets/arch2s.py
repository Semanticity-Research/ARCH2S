...
Author: Cheung Ka Lung (klcheung@mae.cuhk.edu.hk)
...
class ARCH2SDataset(Dataset):
    def __init__(
        self,
        split=("buildings_1", "buildings_2", "buildings_3", "buildings_4"),
               # "buildings_6"),
        data_root="data/arch2s",
        transform=None,
        test_mode=False,
        test_cfg=None,
        cache=False,
        ...
        super(ARCH2SDataset, self).__init__()
    ...
    def get_data(self, idx):
        ...
        if not self.cache:
            # check if valid torch file
            if not torch.load(data_path):
                raise ValueError(f"Invalid torch file: {data_path}")
